...
//spm green is glow strength, glow color is player faction color
//SOFT PCF SHADOWS
...
Cloud 
: register(vs_3_0, c117) <bool unmanaged = 1;>;
...
<string Texture = "NormalMap"; string UIName = "Normal Texture";> =
sampler_state
{
    Texture = <NormalMap>; 
    MinFilter = 3;
...
float EnvMult 
<string UIName = "Reflection Multiplier"; string UIWidget = "Slider"; float UIMax = 1; float UIMin = 0; float UIStep = 0.01;> = { 1 };
...

{
    ...
    temp0.xyz = temp2.xyz * float3(2, 2, 2);
    ...
    //temp0.xyz = temp0.xyz * float3(-1.2, -1.2, 1);
    ...
    tempglow.xyz = temp3.yyy * RecolorColor.xyz *2 ; //add hc zero
    ...
    temp3.xyz = temp4.xyz * float3(2, 2, 2);
...
float4 PS_H_Array_Shader_2(PS_H_Array_Shader_2_Input i) : COLOR //hc no shadow
{
    float4 out_color;
    ...
    tempglow.xyz = temp2.yyy * RecolorColor.xyz *2; //glow channel
...
//    temp3.xyz = temp0.xyz * RecolorColor.xyz + -temp0.xyz;  //fixed bloom
    ...
    temp0.xyz = temp2.yzw * float3(2, 2, 2);
...
float4 PS_H_Array_Shader_3(PS_H_Array_Shader_3_Input i) : COLOR //hc shadow (the only useful one)
{
    ...
    temp3 = tex2D(NormalMapSampler, i.texcoord.xy);
    temp0.xyz = temp3.xyz * float3(2, 2, 0) + float3(-1, -1, 1);
    //temp0.xy = temp0.xy * float2(-1.2, -1.2);
    //temp0.z = sqrt(1 - dot(temp0.xy, temp0.xy)); //normal fix
    temp0.z = sqrt(1 - temp0.x*temp0.x - temp0.y*temp0.y);
...
PixelShader PSCreateShadowMap_Array[1] = {
...
float _CreateShadowMap_Expression21()  //shadow PS
{   
    //    float1 expr0 = AlphaTestEnable.x;
    //    return expr0;
    return 0;
}


float _CreateShadowMap_Expression22()  //shadow VS
{
    float1 expr0;
