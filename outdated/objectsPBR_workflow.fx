...
<string UIName = "GlowColorMax"; string UIWidget = "Color"; > = {0, 0, 0, 2}; 

float4 GlowColorMin  //发光色 波谷，alpha为阵营色
<string UIName = "GlowColorMin"; string UIWidget = "Color"; > = {0, 0, 0, 0}; 
...
static const float MINroughness = { 0.1 }; //最低粗糙度
...
static const float direct_fresnel_cutoff = { 1.5 }; //拟合上面那个用的，1-2之间越小越显著
...
//基于目光与光源，而不是半程向量的菲涅尔效应，虽然不精确但更有表现力，也省一次开方
...
float helper_newdiffuse(float dotLN, float Reflectivity) //corrected with reflectivity
{
    ...
    float lamb_after_fresnel = min(lambertian, (lambertian * lambertian * direct_fresnel_cutoff)); 
    //cheap approx
    //fine approx
