...
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.01 ;> = { 0.4 }; //环境光与天空亮度
...
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.01 ;> = { 1.25 }; //高光（镜面反射）亮度，影响阳光与点光源

float glow_multiply
<string UIName = "glow_multiply"; 
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.1 ;> = { 1.2 }; //发光亮度（spm绿通道标记发光区）

float pointlight_multiply
<string UIName = "pointlight_multiply"; 
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.1 ;> = { 1.8 }; //点光源反射整体亮度

float fix_saturation
<string UIName = "fix_saturation"; 
string UIWidget = "Slider"; float UIMax = 32; float UIMin = 0.1; float UIStep = 0.1 ;> = { 8 }; //反射光谱与自发光的饱和度修复

float roughness
<string UIName = "roughness(microfacet-distribution)"; 
string UIWidget = "Slider"; float UIMax = 1; float UIMin = 0.1; float UIStep = 0.01;> = { 0.25 }; //金属和玻璃的最低粗糙度（太阳倒影的最小光斑尺寸）

bool AlphaTestEnable 
<string UIName = "AlphaTestEnable";> =1; //贴图镂空

// bool AlphaBlendEnable <string UIName = "AlphaBlendEnable";> =1; //会导致基洛夫变透明的BUG
//玻璃菲涅尔效应已删除

bool HCcorrection
<string UIName = "HCcorrection";> =1;  //阵营色加亮
...
string UIWidget = "Slider"; int UIMax = 10; int UIMin = 0;> = { 9 }; //选择哪个颜色为“天空”反射色
...
//get textures
    ...
    if (HCcorrection) {albedo_color = saturate(albedo_color * (1+ spm.z)); }; //HC enhance
    ...
    float3 glowcolor = satfix.rgb * spm.y*spm.y * glow_multiply; //it's additive
    glowcolor = pow(glowcolor, 2) ; //extra gradient

    float3 real_diffusecolor = albedo_color.xyz * (1- spm.x*spm.x) ; //g2 already have dark diffuse for metal
...
//about sun and eye, also nrm backface fix, map lights
...
//ambient light diffuse //DO NOT USE "AmbientLightColor" this thing only exist in VS register!
    float  ground_sky_lerpw = saturate(N.z + 0.5)  ; // (N.z * sharpness + 1)
...
//sunlight spec
    ...
    float3 spec_sunlight = spec_dist * spm.x * f0spectrum.xyz * spec_multiply ; //(spm.x*spm.x)
...
//environmental mirror reflection
    ...
    float3 spec_ambient = fake_skybox_color * spm.x * f0spectrum.xyz *ambient_multiply ; //no spec multiply
...
//final color modify
    ...
    out_color.xyz *= 1- saturate(spm.y*spm.y) ; //minus black body, here lights are
