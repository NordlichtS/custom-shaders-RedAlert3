...
//enhanced for:
// 日冕用的物理材质
//基于objects workflow .fx 做的，没用到岚依的代码，所以风格差别可能有点大
//--------------
...
//
//已完成：
// 用地编光照计算拟合的假天空盒，地平线会根据粗糙度模糊
// 8个点光源全部使用和阳光一样的BRDF算式，感受激光与火焰特效的炫酷倒影吧
//待做：
// 加入天空盒
// 中低画质适配
// MAX预览需要一个点光源,
...
<string UIName = "DiffuseTexture(BASE)";>; //主贴图，循环迷彩
//RGB= 非金属的漫反射，金属的反射光谱，发光区的发光色 A=透明镂空

texture SpecMap 
<string UIName = "SpecMap(SPM)";>; //SPM贴图
//R=粗糙度反转， G= 金属度， B= 阵营色，A=发光

texture NormalMap 
<string UIName = "NormalMap(NRM)";>; //法线贴图
//RG=切线空间 XY 倾斜 ，B 不要放东西，必须涂满蓝或者黑，防止干扰压缩。 A 是AO
//建议用DXT5存储
...
<string UIName = "GlowAmplitude"; float UIMax = 8; float UIMin = 0; float UIStep = 0.25; > 
={ 0 }; 

float GlowPeriod //发光呼吸周期，写0为恒定发光
<string UIName = "GlowPeriod"; float UIMax = 8; float UIMin = 0; float UIStep = 0.2; > 
={ 1 }; 
...
{
    float roughness = lerp(MINroughness, 1, roughmap);
...
float helper_fresnelold(float3 L, float3 V, float F0)
{
    float cosRV = dot( L , V ) ;
...
float helper_newdiffuse(float dotLN, float Reflectivity) //corrected with reflectivity
{
    ...
    float lamb_after_fresnel = lambertian * lambertian ; 
    //cheap approx
    //fine approx
...
float4 spm = tex2D(SpecMapSampler,         i.MainTexUV.xy );
    ...
    dif.xyz *= HCchannelMult ; //这样阵营色没有gamma修正了，可能要互换

    float  Reflectivity = spm.x  ;  // saturate(spm.x + spm.y)
    float  roughness = lerp( dielectrics_roughness , (1-spm.x) , metalness) ;
    float  glossiness = helper_glossiness (roughness) ; //1-8
    float3 speccolor = lerp((float3(1,1,1) * Reflectivity), dif.xyz, metalness);

    float3 difcolor = dif.xyz * (1 - metalness) ;
    //difcolor *= 1 - FresnelF0 * Reflectivity; //大概在base贴图上是没有这个修正的
    float  difAO = nrm.w * nrm.w ;  //mult on env dif
    float  mirAO = lerp( 1 , nrm.w , roughness);  //mult on env spec
...
*/
    ...
    float glowmap = (spm_w_glow_invert)? (1- spm.w) : spm.w ;
    if(HasRecolorColors)
    { out_color.xyz += dif.xyz * glowmap * helper_glowpulse(); };
