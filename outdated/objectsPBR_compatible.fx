...
<string UIName = "GlowPeriod(sec,0=ForbidGlow)"; float UIMax = 10; float UIMin = 0; float UIStep = 0.2; > ={ 2 }; 
...
<string UIWidget="None"; bool unmanaged = 1;> = { 0.2, 0.2, 0.2 }; //string SasBindAddress = "Sas.AmbientLight[0].Color"; 
...
float2 helper_vertextweak(float texUVY, float old_alpha)
{
    ...
    float scan_interval = 0.1 ; float print_height = 512 ;
    ...
    addheight = saturate(1 - newalpha) * print_height;
    newalpha = clamp(newalpha , -1, 1) ;
...
VS_H_Array_Shader_1_Output VS_H_Array_Shader_1(VS_H_Array_Shader_1_Input i)  //have bones skin
{
    ...
    temp0.y = 1.0f / temp0.x;
    o.texcoord5.w = temp0.x;
    o.texcoord5.xyz = temp1.xyz * temp0.yyy + float3(0, 0, -0.0025); //0.0015
...
float helper_glowpulse()
{
    ...
    //if(GlowPeriod ==0){phase = 1 ;}; //ALREADY SET MINIMUM TO 0.2
    phase = abs(phase *2 -1) ;
    phase = phase *2 -1 ; //swing between -1 and +1
    float LumineMult = (phase * GlowAmplitude) +1 ;
...
// MOVING CLOUD REFLECTION    - mix type - sky color - reflect vector - i.FogCloudUV.zw - glossiness
...
//environmental stuff
    ...
    float3 ground_color = float3(0,0,0) + i.color.xyz ; //cannot move to final mix
...
//final color modify
    ...
    //out_color.xyz = i.color.xyz ; //this will make cavity glow !
...
//#if defined(IS_BUILDING_SHADER)
    ...
    float4 dmgtex = tex2D(DamagedTextureSampler, i.MainTexUV.wz );
    if(IgnoreDamageTex){dmgtex = float4(1,1,1,0);};
...
technique MAXpreview
{
    pass p0 
    {
        ...
        AlphaFunc = 7;
        AlphaRef = 64;
        AlphaTestEnable = 0;//(AlphaTestEnable) ;
        AlphaBlendEnable = 0 ;
    }
}
