...
//float maxout
//<string UIName = "maxout(max_output_brightness)";
//string UIWidget = "Slider"; float UIMax = 16; float UIMin = 0; float UIStep = 0.1 ;> = { 5.0 }; //avoid super bloom
...
3= accent MAX
4= accent MIN
5= hard coded zero black
6= hard coded pure white
7= ambient color
8= sum of two accent lights
9= ambient + accent MAX
10= ambient + both accent
*/

float ambient_multiply
<string UIName = "ambient_multiply"; string SasBindAddress = "Sas.pbr_ambient_multiply";
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.01 ;> = { 0.8 }; //total ambient light color

float diffuse_multiply
<string UIName = "diffuse_multiply"; 
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.01 ;> = { 1.0 }; //diffuse brightness (sunlight and pointlight)

float spec_multiply
<string UIName = "spec_multiply"; 
string UIWidget = "Slider"; float UIMax = 4; float UIMin = 0; float UIStep = 0.01 ;> = { 2.8 }; //specular brightness (sunlight and pointlight)
...
string UIWidget = "Slider"; float UIMax = 1; float UIMin = 0.1; float UIStep = 0.01;> = { 0.16 }; //minimal roughness on metal+glass (affect sun mirror radius)

float glassf0
<string UIName = "glassf0(fresnel-decay)"; 
string UIWidget = "Slider"; float UIMax = 1; float UIMin = 0; float UIStep = 0.01;> = { 0.16 }; //fresnel effect on glass, darker = stronger
...
//<string UIName = "ignore_vertex_alpha";> =0 ; // fix building "damage texture" bug. 
//Do not turn on for stealth vehicles (it will lose the semi-transparent stealth effect)
...
struct{float3 Color;float3 Direction;} DirectionalLight[3] : register(vs_2_0, c5) : register(ps_3_0, c5) : register(vs_3_0, c5) <bool unmanaged = 1;> = 
...
int     NumPointLights  <string SasBindAddress = "Sas.NumPointLights"; string UIWidget = "None";> =8; //: register(ps_3_0, i0)
struct{float3 Color;float3 Position;float2 Range_Inner_Outer;} PointLight[8] : register(ps_3_0, c89) <bool unmanaged = 1;>;
struct{float4 WorldPositionMultiplier_XYZZ;float2 CurrentOffsetUV;} Cloud : register(vs_3_0, c117) <bool unmanaged = 1;>;
...
float4  Shadowmap_Zero_Zero_OneOverMapSize_OneOverMapSize : register(ps_3_0, c11) 
...
sampler2D DamagedTextureSampler <string Texture = "DamagedTexture"; string UIName = "Damaged Texture";> =
sampler_state{
    Texture = <DamagedTexture>; 
    MinFilter = 2;
    MagFilter = 2;  
...
VS_H_Array_Shader_0_Output VS_H_Array_Shader_0(VS_H_Array_Shader_0_Input i)
{
    ...
    o.texcoord5.xyz = temp0.xzw * temp0.yyy + float3(0, 0, -0.003);
...
VS_H_Array_Shader_1_Output VS_H_Array_Shader_1(VS_H_Array_Shader_1_Input i)
{
    ...
    temp0.y = 1.0f / temp0.x;
    o.texcoord5.w = temp0.x;
    o.texcoord5.xyz = temp1.xyz * temp0.yyy + float3(0, 0, -0.003);
...
{
    float4 texcoord  : TEXCOORD; //float2
    float3 texcoord1 : TEXCOORD1;
    float3 texcoord2 : TEXCOORD2;
    float3 texcoord3 : TEXCOORD3;
    float3 texcoord4 : TEXCOORD4;
...
{
    float4 out_color = i.color.xyzw; //actually useless
...
//get textures
    ...
    float4 dmgtex   = tex2D(DamagedTextureSampler, i.texcoord.wz);

    out_color.w = texcolor.w;

    if (! HasRecolorColors) {spm.z =0 ;};
    if (GAMMAcorrection) { texcolor.xyz *= texcolor.xyz ; dmgtex.xyz *= dmgtex.xyz ;};

    spm.y = min (spm.y,  (1- glassf0)) ; //limit maximum "glassness"
    float insulentf0 = 1- spm.y ;  //glass judge. it's now fresnel f0 for everything
    float glass_extra = 1+ spm.y*glassglow ; 
    ...
    float3 real_diffusecolor = albedo_color.xyz * (1- spm.x) ; //no glass here, no ao for glass
...
//ambient light diffuse //DO NOT USE "AmbientLightColor" this thing only exist in VS register!
    ...
    float  ground_sky_lerpw = saturate(N.z + 0.5)  ; 
...
//sunlight diffuse
    float3 diffuse_sunlight = real_diffusecolor.xyz * sun_tilt * diffuse_multiply * glass_extra ; 
...
//sunlight spec
    ...
    float3 spec_sunlight = spec_dist * FresnelV * spec_howbright * f0spectrum.xyz * spec_multiply *glass_extra; 
...
//environmental mirror reflection
    ...
    float3 spec_ambient = fake_skybox_color * FresnelV * spec_howbright * f0spectrum.xyz * ambient_multiply * glass_extra; 
...
//point lights
    ...
    for (int countpl = 0; countpl < maxPLcount; ++countpl ) {
        ...
        float3 spec_pl = spec_multiply * spm.x * pl_specdist * f0spectrum.xyz ; //no fresnel

        float3 thispl_total = (diffuse_pl + spec_pl) * thispl_COLOR.xyz ;
        pl_total += thispl_total.rgb ;
    };
    pl_total *= pointlight_multiply ;
...
//final color modify
    ...
    out_color.xyz *= lerp( float3(1,1,1) , RecolorColor  , spm.z); //haha i put HC here
    out_color.xyz *= lerp( float3(1,1,1) , TintColor.xyz , (1.25- EYEtilt*EYEtilt) ); 

    out_color.xyzw *= lerp(dmgtex.xyzw, float4(1,1,1,1), i.color.w);  //crude solution for holes
...
{
    float4 out_color;
    float4 dmgtex = tex2D(DamagedTextureSampler, i.texcoord2.xy);
    float opacity = lerp(dmgtex.w, float(1) , i.color.x);
    //clip(opacity <0.4);
    if(opacity <0.5) {discard;};
    out_color = i.texcoord1.x;
    return out_color;
}  //so why doesnt light pass through the holes


PixelShader PSCreateShadowMap_Array[1] = {    compile ps_2_0 PSCreateShadowMap_Array_Shader_1(), };
