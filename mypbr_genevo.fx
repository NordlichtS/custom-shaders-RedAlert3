...
string UIWidget = "Slider"; float UIMax = 1; float UIMin = 0; float UIStep = 0.01;> = { 0.12 }; //fresnel effect on glass, darker = stronger
...
//ambient light diffuse //DO NOT USE "AmbientLightColor" this thing only exist in VS register!
    ...
    // ground_color = lerp (ground_color, sky_color, (spm.y*glassglow) ); //avoid side window superdark
...
//sunlight diffuse
    float3 diffuse_sunlight = real_diffusecolor.xyz * sun_tilt * diffuse_multiply *(1+ spm.y*glassglow) ; //glass diffuse only affect by sun
...
//sunlight spec
    ...
    float3 spec_sunlight = spec_dist * FresnelV * spec_howbright * f0spectrum.xyz * spec_multiply ; //(spm.x*spm.x)
...
//environmental mirror reflection
    ...
    float3 spec_ambient = fake_skybox_color * FresnelV * spec_howbright * f0spectrum.xyz *ambient_multiply ; 
...
//final color modify
    ...
    out_color.xyz = min(out_color.xyz ,  float3(maxout, maxout, maxout)) ;
    return out_color;
}
